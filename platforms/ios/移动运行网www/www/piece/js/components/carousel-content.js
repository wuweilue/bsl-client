        ...
        initialize: function() {
            ...
            var thisCarousel =  Swipe(me.el,{
                ...
                callback:function(index,el){
                    ...
                    if(!isSeqLoop){
                        contentIndex = titleCount - index - 1;
                    }

                    var offLeft = Math.floor($(pagerEl.find('td')[contentIndex]).offset().left);
                    $(pagerEl.find('.titlePointer')).animate({
                        left:offLeft
                    },300,'linear',function(){

                        $(pagerEl.find('td')).removeClass('activate');
                        $(pagerEl.find('td')[contentIndex]).addClass('activate');
                    });
                }
            });
            ...
            for(k = 0; k < pCount; k++){
                ...
                $(pagerEl.find('td')[sliderIndex]).on('click',function(){
                    ...
                    // alert(sliderIndex)
                    var pageIndex = $(this).index();//pCount-1 - $(this).index();
...
// 
                ...
                for(var j=0;j<contentLength;j++){
                    var contentIndex = 0;
                    if(!isSeqLoop){
                        contentIndex = contentLength-j-1;
                    }
                    $(swipContainer).append( 
                        $($(tag).children()[contentIndex])
                        .css({'width':windowWidth})
                     );

                    new iScroll($($(swipContainer).children()[j]));
                }
