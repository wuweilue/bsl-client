$.fn.listSliderNavFullWindow = function(containerId,options,thisIscroll,storgeKey) {
	...
	var listSlider = $(this).find('.listSlider');
	...
	var upSideDownButton_Height=45;
	var navButtonHeihgt=45;
	var cancelButtonHeihgt=0;//70;



	$('.listSlider-nav', listSlider).css({'height':$(window).height()-upSideDownButton_Height*2-navButtonHeihgt-cancelButtonHeihgt});
	...
	if(options.listSliderHeight){
		height = options.listSliderHeight;
	}else{
	...
	var me = this
	$('.listSlider-content ul li a.cityName', listSlider).click(function(e){
		
		var value = $(e.toElement).text();
		var targetId = Piece.Session.loadObject(storgeKey+"Target");
		
		Piece.Session.saveObject(storgeKey+targetId,value);

		window.history.back();// = "../com.csair.base/index.html?cube-action=pop#com.csair.base/cityBlock"
	...
	function refreshHeight(){
		var newHeight = $(window).height()-upSideDownButton_Height*2-navButtonHeihgt;//-cancelButtonHeihgt;
	...
	var sliderObject = {
		...
		hide:function(){$(containerId).hide();},
		refresh:function(){thisIscroll.refresh();},
		slider:function(){return listSlider;},
