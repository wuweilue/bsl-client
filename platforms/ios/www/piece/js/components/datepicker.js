define(['backbone', 'underscore', 'zepto'], function(Backbone, _, $) {
    ...
    var Calendar = Backbone.View.extend({
        initialize: function() {
            ...
            var isShowInitial = $(this.el).attr('data-showInit');
            dataDate = (dataDate === null||dataDate.length===0)?today:dataDate;
            ...
            datepickerHtml = datepickerHtml.calendar({
                date: ((dataDate === null||dataDate.length===0) ? today : dataDate),
            ...
            var initDate = '';
            if(isShowInitial){ initDate = $.calendar.formatDate(dataDate);}
             $(me.el).val(initDate);
            ...
            $(datepickerHtml).find('.ui-calendar-calendar').parent().on("click",function(e){
                if(e.toElement.className=='ui-calendar'){
                    datepickerHtml.hide();
                }
            });
