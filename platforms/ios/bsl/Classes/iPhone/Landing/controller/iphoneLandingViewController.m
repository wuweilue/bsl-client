...
- (void)launch:(int)index identifier:(NSString *)identifier{
    ...
    //植入界面
     ...
     __block CubeWebViewController *aCubeWebViewController  = [[CubeWebViewController alloc] init];
...
- (void)viewDidLoad
{
    ...
    if (iPhone5) {
        backgroundView =[[[UIImageView alloc]initWithFrame:CGRectMake(0,0, 320, 1136/2)]autorelease];
    }else{
        backgroundView =[[[UIImageView alloc]initWithFrame:self.view.frame]autorelease];
    }
...
//    [self initPageControl];
    ...
    //添加双击事件
    UITapGestureRecognizer* doubelTap = [[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(doubleTap)]autorelease];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithCustomView:navLeftButton] autorelease];
    
    [navLeftButton release];
...
-(void)initPatch
{
    ...
    if(desktopIconEnable){
        ...
        self.itemsTableContainer.tableHeaderView = [self initSingleSearchBar];
    }
...
-(UIView *)initSingleSearchBar{
    if(self.searchBarView == nil){
        if(desktopIconEnable){
            self.searchBarView = [self getSearchBarViewSwitchSelector:@selector(changeViewSight:) andIsRightBtnActive:NO];
        }else{
            self.searchBarView = [self getSearchBarViewSwitchSelector:@selector(changeViewSight:) andIsRightBtnActive:YES];
        }
    }else{
...
-(void)activateDownloadSrceen{
    ...
    [aDownloadViewController release];
...
-(void)changeViewSight:(id)sender
{
    ...
    if (!desktopIconEnable) {
        ...
        [self.backContentView addSubview:_itemsContainer];
        [self.backContentView addSubview:[self initSingleSearchBar]];
        [_itemsTableContainer removeFromSuperview];
...
- (void)dealloc {
    ...
    [_itemsContainer release];
    [_pageControl release];
    [_currentIconDic release];
    ...
    if (desktopIconEnable) {
        ...
        [self.navigationItem setRightBarButtonItem:
                                        ...
                                        initWithBarButtonSystemItem:UIBarButtonSystemItemDone
                                                    target:self action:@selector(DoneEditing)] autorelease]  animated:YES] ;
    }
...
-(void)drawWithDataSource:(NSMutableDictionary*)tempDic
{
    ...
    if(desktopIconEnable){
        [self.itemsContainer addSubview:[self initSingleSearchBar]];
    }else{
        [self.itemsTableContainer reloadData];
    }
