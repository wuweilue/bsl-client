...
-(void)chatPanelDidSend:(ChatPanel *)__chatPanel{
    ...
    if([content length]>0){
        [chatLogic sendMessage:content chatWithUser:self.chatWithUser];
    }
    //把输入框清空
    __chatPanel.text = @"";
...
-(void)chatPanelRecordTouch:(ChatPanel *)chatPanel isTouch:(BOOL)touch{
    if(touch){
        ...
        [chatLogic sendVoice:[Base64 stringByEncodingData:fileData] urlVoiceFile:recorder.recordFile chatWithUser:self.chatWithUser];
...
-(void)recordDidPlayError:(Recorder*)__recorder{
    ...
    [SVProgressHUD showErrorWithStatus:@"文件不存在" duration:0.5f];
    playingIndex=-1;
    ...
    if ([mediaType isEqualToString:@"public.image"]){
        ...
        [chatLogic uploadImageToServer:image finish:^(NSString* fileId,NSString* path){
            if([fileId length]>0){
                ...
                [chatLogic sendfile:fileId path:path chatWithUser:self.chatWithUser];
            }
            else{
                [SVProgressHUD dismissWithError:@"图片上传失败" afterDelay:1.0f];
            }
        }];
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath{
    if ([anObject isKindOfClass:[MessageEntity class]]&&type==NSFetchedResultsChangeInsert) {
        ...
        if(userInfo!= nil){
            ...
            userInfo.userMessageCount = [NSNumber numberWithInt:0];
