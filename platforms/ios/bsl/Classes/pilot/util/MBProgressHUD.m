...
//	id me = [self initWithFrame:view.bounds];
    ...
    id me = [self initWithFrame:CGRectMake(-view.bounds.size.width/2, -view.bounds.size.height/2, view.bounds.size.width*2, view.bounds.size.height*2)];
...
//            detailsLabel.numberOfLines = 0;
			...
			//CGFloat maxHeight = frame.size.height - self.height - 2*margin;
//			CGSize labelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) lineBreakMode:detailsLabel.lineBreakMode];
            lHeight = customButton.frame.size.height;
            lWidth = customButton.frame.size.width;
...
//            lFrame.origin.y -= (floorf(lHeight / 2 + PADDING / 2));
            ...
            CGRect lFrameD = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                        lFrame.origin.y + lFrame.size.height + 15, lWidth, lHeight);
            customButton.frame = lFrameD;
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
	...
	if (self.superview) {
		self.bounds = CGRectMake(0, 0, self.superview.bounds.size.width*2, self.superview.bounds.size.height*2);
		[self setNeedsDisplay];
	}
